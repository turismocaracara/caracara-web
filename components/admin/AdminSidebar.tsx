...
// ── SVG icon components (Heroicons 2 outline, 24-viewbox, rendered at 17px) ──
...
const NAV_SECTIONS = [
  ...
  },
  {
    label: 'Contenido',
    items: [
      { href: '/admin/tours', label: 'Tours', icon: <IcMap /> },
      { href: '/admin/vans',  label: 'Vans',  icon: <IcTruck /> },
      { href: '/admin/equipo', label: 'Equipo', icon: <IcUsers /> },
    ],
  },
  {
    label: 'Sistema',
    items: [
      { href: '/admin/reportes', label: 'Reportes',      icon: <IcChartBar /> },
      { href: '/admin/config',   label: 'Configuración', icon: <IcCog /> },
    ],
  },
];
      ...
      <nav className="flex-1 py-3 px-2 flex flex-col gap-0 overflow-y-auto">
        {NAV_SECTIONS.map((section, si) => (
          <div key={section.label}>
            {si > 0 && <hr className="border-gray-100 my-2 mx-1" />}
                ...
                return (
                  <Link
                    ...
                    className={`relative flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${
                      active
                        ? 'bg-teal/[0.08] text-teal font-medium'
                        : 'text-gray-600 hover:bg-gray-50 hover:text-gray-800'
                    }`}
                  >
                    {active && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-teal rounded-r" />
                    )}
                    {icon}
                    {label}
                  </Link>
